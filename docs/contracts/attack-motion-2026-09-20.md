...
文字每次攻击先取消旧动作并归零，销毁时清理补间。赵云“七进七出”的虚影命中不重复驱动本体文字。文字层不接收指针事件，两个武将字仍可独立点击查看原单位。

2026-09-20 上线后核实：用户浏览器 `prefers-reduced-motion: reduce` 为真，旧版因此跳过本体动作。用户明确要求无需设置、直接开启：士兵姿势与文字身体/手脚属于必要攻击反馈，始终播放；不新增设置项、不修改系统偏好。原有低动效分支继续限制镜头震动、环境摆动及装饰特效。动画入口保留旧 reduced 参数作为兼容参数，但不再用于禁用攻击姿势。需要在 reduce 环境重新验证攻击者像素变化、双端持续攻击和拖动性能。
...
## 默认启用攻击动作补丁验收（2026-09-20）

- 原始发布 `32314bc` 已上线；此补丁按用户“不要设置，直接开启”要求更新攻击动作，不涉及服务器、战斗规则或新增网络流量。
- `pnpm verify` 全部通过，197 项测试；JS 1,679,235 字节，低于原构建且未放宽预算。
- `QA_REDUCED_MOTION=1 QA_HEADED_PERF=1` 下运行 `attack-motion.spec.ts realtime-rendering.spec.ts`，桌面与 390×844 手机尺寸 26 项通过，包括四兵种攻击者姿势、双字武将点击、连续攻击、双模式压力拖动。
- 32 敌人、10Hz 快照：桌面形象/文字均 60.2 FPS，手机尺寸形象/文字 60.2 / 60.5 FPS。仅代表当前电脑浏览器与模拟尺寸，未声称实体低端手机结果。
- 截图/录像：`artifacts/attack-motion/always-on-qa/`；低动效开启时字身起势、四肢出招与归位多帧已目视核对。兼容参数保留，未增加玩家设置。
