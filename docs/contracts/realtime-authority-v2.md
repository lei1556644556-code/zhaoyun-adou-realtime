...
- 合同版本：`2.4.0`（客户端本地确定性推进、服务端命令裁决与稀疏纠正快照）
- 提供方：06 实时联机
- 消费方：02 权威战斗内核、03 战场 UI、05 账号存档、07 验收、08 发布
- 协议/规则版本：握手必须同时校验 `protocolVersion=0.6.0`、`rulesetVersion=1.0.9`、规则配置结构版本和快照版本。
...
客户端只发送 `GameCommand`。真人双方与服务端都调用同一份共享确定性内核：客户端负责连续 Tick 和画面所需的即时状态，服务端负责命令合法性、顺序、最终胜负与纠正检查点。客户端不得自行生成随机结果或绕过服务端接受命令；人机模式实现同一个端口，但完全在本地运行。
...
建议使用 Supabase Postgres 表 `match_rooms`、`match_seats`、`match_commands`。服务端使用私有 service-role 凭证；该凭证绝不能进入 Pages 构建。战斗中每 10 秒写恢复检查点，断线、结算和进程退出立即写入；服务重启后从最后快照续跑。硬故障时最多回退一个持久化周期，但不得以每 Tick 或每命令落库牺牲当局流畅度。
...
2. `authorityHealthy`：最近 6 秒是否收到权威心跳、已接受命令或纠正检查点；
3. `opponentConnected`：服务端最后一次确认对手席位在线。

断线保留席位 120 秒；期间权威模拟继续，重连成功后发送最新完整快照。恢复失败不得自动删除本地恢复令牌或最近快照，只有玩家明确“退出本局”才清理。

## 实时同步与带宽

- 双端客户端以 10Hz 在本机推进共享模拟，渲染不等待网络快照。
- 服务端校验并排序命令后，以 `match:command-applied` 向双方广播轻量命令；客户端只重放这类已被接受的命令。
- 服务端最多每 5 秒发送一次压缩的 `match:checkpoint`，用于纠正时钟漂移与漏包；开局、恢复和主动校正使用完整 `match:snapshot`。
- 检查点按房间错峰，避免多个房间同时发送大包造成瞬时拥塞。
- 客户端本地重放失败时发送 `match:resync`，在收到完整快照前保留当前画面与恢复凭证。
