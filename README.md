...
GitHub Pages 只托管静态前端；Supabase Auth 与 Postgres 只负责账号、云存档和权威房间检查点。创建房间、加入房间和随机匹配全部连接常驻 `apps/server`，伤害、随机、合成、波次和胜负只由服务端裁决。
...
- 人机对战、创建房间、输入房号加入、随机匹配；在线对局由常驻 Socket.IO 服务端权威模拟。
...
迁移会创建 `zhaoyun_adou_profiles` 账号云存档表，以及仅 service role 可访问的 `zhaoyun_adou_matches` 权威房间检查点表；两个表都启用 RLS，浏览器无法直接读取对局恢复令牌或服务端快照。客户端由环境变量提供项目配置。开发者可复制到 `apps/client/.env.development.local`；正式 Pages 发布由仓库变量注入：

```bash
VITE_SUPABASE_URL=https://你的项目.supabase.co
VITE_SUPABASE_PUBLISHABLE_KEY=你的PublishableKey
VITE_SERVER_URL=https://你的常驻权威服务.example
```
